...
export function resetForm(addFirst = true, dpsData = []) {
    ...
    // Prepara as opções de DPS kA
    if (dpsData && dpsData.length > 0) {
        const uniqueKas = [...new Set(dpsData.map(d => d.corrente_ka))].sort((a,b) => a-b);
        dpsKaOptions = uniqueKas.map(ka => `<option value="${ka}">${ka} kA</option>`);
    }

    populateDpsKaDropdown('feederDpsKa');
...
export function addCircuit() {
    ...
    populateDpsKaDropdown(`dpsKa-${circuitCount}`);

    initializeCircuitListeners(circuitCount);
}

function populateDpsKaDropdown(selectId) {
    const select = document.getElementById(selectId);
    if (select) {
        select.innerHTML = '<option value="">-</option>' + dpsKaOptions.join('');
    }
}
...
export function renderReport(calculationResults){
    ...
    const dadosCliente = circuitResults[0]?.dados || feederResult.dados;
    ...
    allCalculations.forEach(result => {
        ...
        if(dados.id !== 'Geral') { // Fatores K não se aplicam ao alimentador da mesma forma
