...
export function resetForm(addFirst = true, dpsOptions = []) {
    ...
    circuitCount = 0;
    if (addFirst) addCircuit(dpsOptions);
}

export function addCircuit(dpsOptions = []) {
    circuitCount++;
    ...
    const dpsSelect = document.getElementById(`dps-${circuitCount}`);
    dpsSelect.innerHTML = '<option value="">Nenhum</option>'; 
    if (dpsOptions) {
        dpsOptions.forEach(dps => {
            const option = document.createElement('option');
            option.value = dps.id;
            option.textContent = `${dps.classe} - ${dps.modelo}`;
...
function initializeCircuitListeners(id) {
    ...
    const fatorDemanda = document.getElementById(`fatorDemanda-${id}`);
...
export function populateFormWithProjectData(project, dpsOptions = []) {
...
export function renderReport(results){
    if(!results || !results.circuitos || results.circuitos.length === 0) return;
    
    const { circuitos, alimentacaoGeral } = results;
    const dataHora = (new Date).toLocaleString('pt-BR');
    const formatLine = (label, value) => (label + ':').padEnd(35, ' ') + value;
    let reportText = `======================================================\n==           RELATORIO DE PROJETO ELETRICO           ==\n======================================================\n${formatLine('Gerado em', dataHora)}\n`;
    
    const dadosCliente = circuitos[0].dados;
    ...
    circuitos.forEach(result => {
        reportText += `${formatLine(`Circuito ${result.dados.id}`, `${result.dados.nomeCircuito} - ${result.calculos.potenciaDemandada.toFixed(2)} W`)}\n`;
    });
    reportText += `------------------------------------------------------\n`;
    reportText += `${formatLine('Potência Total Demandada', `${alimentacaoGeral.calculos.potenciaTotalDemandadaW.toFixed(2)} W`)}\n`;
    reportText += `${formatLine('Corrente Total Demandada', `${alimentacaoGeral.calculos.correnteTotalDemandada.toFixed(2)} A`)}\n`;
    ...
    circuitos.forEach(result => {
        ...
        let dpsText = 'Nao';
        if (dados.dpsInfo) {
            dpsText = `Sim, ${dados.dpsInfo.classe} (${dados.dpsInfo.corrente_ka} kA)`;
        }
        reportText += `${formatLine('Protecao DPS', dpsText)}\n`;

        reportText += `${formatLine('Eletroduto (aprox.)', `${calculos.dutoRecomendado} (${calculos.numCondutores} condutores)`)}\n`;
    });

    reportText += `\n\n======================================================\n==      MEMORIAL DE CALCULO - ALIMENTAÇÃO GERAL      ==\n======================================================\n`;
    reportText += `${formatLine('Potência Total Demandada', `${alimentacaoGeral.calculos.potenciaTotalDemandadaW.toFixed(2)} W`)}\n`;
    reportText += `${formatLine('Corrente Total Demandada', `${alimentacaoGeral.calculos.correnteTotalDemandada.toFixed(2)} A`)}\n`;
    reportText += `${formatLine('Disjuntor Geral Recomendado', `${alimentacaoGeral.calculos.disjuntorRecomendado.nome}`)}\n`;
    reportText += `${formatLine('Cabo de Entrada Recomendado', `${alimentacaoGeral.calculos.bitolaRecomendada}`)}\n`;
    reportText += `${formatLine('Queda de Tensão Calculada', `${alimentacaoGeral.calculos.quedaTensaoCalculada.toFixed(2)} %`)}\n`;
    reportText += `${formatLine('Limite de Queda de Tensão', `${alimentacaoGeral.dados.limiteQuedaTensao.toFixed(2)} %`)}\n`;
...
export function generatePdf(results, currentUserProfile) {
    ...
    // ... (resto da geração do PDF, sem alteração na lógica, mas usando a nova estrutura)
