...
function populateTemperatureDropdown(selectElement, temperatures) {
    ...
    if (temperatures.includes(parseInt(currentValue))) { selectElement.value = currentValue; }
    else if (temperatures.includes(30)) { selectElement.value = '30'; }
    else if (temperatures.length > 0) { selectElement.value = temperatures[0]; }
}
...
function updateFeederPowerDisplay() {
    // Esta função precisará de uma lógica mais complexa no futuro para somar as demandas dos QDCs principais.
    // Por enquanto, vamos manter a soma simples.
    ...
    // A potência demandada será calculada pelo back-end.
...
function getQdcHTML(id) {
    return `
    <div class="qdc-block" id="qdc-${id}" data-id="${id}">
        <div class="qdc-header">
            <div class="qdc-header-left">
                <input type="text" id="qdcName-${id}" value="QDC ${id}" class="qdc-name-input">
            </div>
            <div class="qdc-header-center">
                <label for="qdcParent-${id}">Alimentado por:</label>
                <select id="qdcParent-${id}" class="qdc-parent-select"></select>
            </div>
...
export function addQdcBlock() {
    qdcCount++;
    const newQdcDiv = document.createElement('div');
    newQdcDiv.innerHTML = getQdcHTML(qdcCount);
    ...
    addCircuit(qdcCount); // Adiciona um circuito inicial ao novo QDC
}
...
function updateQdcParentDropdowns() {
    ...
    allQdcs.forEach(qdc => {
        ...
        // Filtra para não permitir que um QDC seja pai de si mesmo
        const filteredOptions = options.filter(opt => !opt.includes(`value="${id}"`));
...
// --- LÓGICA DE CIRCUITO (ADAPTADA PARA QDCs) ---

export function addCircuit(qdcId) {
    circuitCount++;
    ...
    return `<div class="circuit-block" id="circuit-${id}" data-id="${id}">
        ...
        <div class="circuit-content">
            <div class="form-grid">
                ...
                <div class="checkbox-group"> <input type="checkbox" id="requerDR-${id}"><label for="requerDR-${id}">Requer Proteção DR</label> </div>
            </div>
        </div>
    </div>`;
}
...
function drawDisjuntor(x, y, text, fases = 'Monofasico') {
    let symbolPath = '';
    switch (fases) {
        case 'Trifasico': symbolPath = `<path d="M ${x - 5} ${y - 2} q 5 -10 10 0 M ${x - 5} ${y + 2} q 5 -10 10 0 M ${x - 5} ${y + 6} q 5 -10 10 0" stroke="black" stroke-width="1.5" fill="none" />`; break;
        case 'Bifasico': symbolPath = `<path d="M ${x - 5} ${y} q 5 -10 10 0 M ${x - 5} ${y + 4} q 5 -10 10 0" stroke="black" stroke-width="1.5" fill="none" />`; break;
        default: symbolPath = `<path d="M ${x - 5} ${y + 2} q 5 -10 10 0" stroke="black" stroke-width="1.5" fill="none" />`; break;
    }
    return `<g text-anchor="middle"> <circle cx="${x - 12.5}" cy="${y}" r="1.5" fill="black"/> <circle cx="${x + 12.5}" cy="${y}" r="1.5" fill="black"/> ${symbolPath} <text x="${x}" y="${y - 18}" style="font-family: Arial; font-size: 11px;">${text}</text> </g>`;
}
function drawDR(x, y, text, fases = 'Monofasico') {
    const drColor = '#27ae60';
    let internalSymbol = '';
    if (fases === 'Trifasico') { internalSymbol = `<path d="M ${x-4} ${y-5} v 10 M ${x} ${y-5} v 10 M ${x+4} ${y-5} v 10 M ${x-4} ${y-5} h 8" stroke="${drColor}" stroke-width="1" fill="none"/>`; } 
    else { internalSymbol = `<path d="M ${x} ${y-5} v 10 M ${x-3} ${y-5} h 6" stroke="${drColor}" stroke-width="1" fill="none"/>`; }
    return `<g text-anchor="middle"> <rect x="${x - 12.5}" y="${y - 12.5}" width="25" height="25" stroke="${drColor}" stroke-width="1.5" fill="white" /> <text x="${x}" y="${y - 18}" style="font-family: Arial; font-size: 10px; fill: ${drColor};">${text}</text> <text x="${x}" y="${y+4}" style="font-family: Arial; font-size: 11px; font-weight: bold; fill: ${drColor};">DR</text> ${internalSymbol} </g>`;
}
function drawDPS(x, y, feederData) {
    const dpsColor = '#27ae60';
    let numDPS = feederData.fases === 'Monofasico' ? 2 : (feederData.fases === 'Bifasico' ? 3 : 4);
    const dpsInfo = feederData.dpsInfo;
    const text = dpsInfo ? `${numDPS}x DPS Cl.${dpsInfo.classe} ${dpsInfo.corrente_ka}kA` : `${numDPS}x DPS`;
    return `<g> <rect x="${x - 45}" y="${y - 12.5}" width="90" height="25" stroke="${dpsColor}" stroke-width="1.5" fill="white" /> <text x="${x}" y="${y + 4}" text-anchor="middle" style="font-family: Arial; font-size: 10px; fill:${dpsColor};">${text}</text> <line x1="${x}" y1="${y + 12.5}" x2="${x}" y2="${y + 30}" stroke="black" stroke-width="1" /> ${drawGroundSymbol(x, y + 30)} </g>`;
}
...
function drawConductorSymbol(x, y, numConductors) { let path = ''; for (let i = 0; i < numConductors; i++) { path += ` M ${x - 5} ${y + 5 + (i * 4)} l 10 -5`; } return `<path d="${path}" stroke="black" stroke-width="1" fill="none" />`; }
...
function buildUnifilarSvgString(calculationResults) {
    ...
    const yStart = 40, yBusbar = yStart + 150, circuitWidth = 100, marginLeft = 60, totalCircuits = circuitResults.length, svgWidth = (totalCircuits * circuitWidth) + marginLeft * 2, svgHeight = 600;
