// Arquivo: ui.js (COMPLETO E CORRIGIDO - Com logging adicional para BTU/CV)
...
function _internal_updateQdcParentDropdowns() {
    ...
    selects.forEach(select => {
        ...
        const initialValue = select.dataset.initialParent || currentValue;
        select.innerHTML = '';
...
function handleCircuitTypeChange(id) {
    ...
    if (btuGroup) {
        const shouldShowBtu = (tipo === 'ar_condicionado');
        btuGroup.classList.toggle('hidden', !shouldShowBtu);
        console.log(`handleCircuitTypeChange (${id}): BTU Group hidden = ${!shouldShowBtu}`);
    }
    if (cvGroup) {
        const shouldShowCv = (tipo === 'motores');
        cvGroup.classList.toggle('hidden', !shouldShowCv);
        console.log(`handleCircuitTypeChange (${id}): CV Group hidden = ${!shouldShowCv}`);
    }
...
export function populateFormWithProjectData(project) {
    ...
    if (project.qdcs_data && Array.isArray(project.qdcs_data) && qdcContainerTarget) {
        ...
        function visit(qdcId) { 
            ...
            if(!sortedQdcs.some(sq => sq.id == qdc.id)) { 
                 sortedQdcs.push(qdc);
            }
        }
        ...
        sortedQdcs.forEach(qdc => {
            const renderedQdcId = addQdcBlock(String(qdc.id), qdc.name, qdc.parentId, fragment); 
        ...
        sortedQdcs.forEach(qdc => {
            ...
            initializeQdcListeners(renderedQdcId); 
...
export function populateUsersPanel(users) {
    ...
    users.forEach(user => {
        ...
        const blockButtonClass = user.is_blocked ? 'btn-green' : 'btn-orange'; 
