...
function initializeCircuitListeners(id) {
    ...
    const fatorDemandaSelect = document.getElementById(`fatorDemanda-${id}`);
    ...
    const handleCircuitTypeChange = () => {
        ...
        const isHeatingCircuit = (selectedType === 'aquecimento');
        for (const option of fatorDemandaSelect.options) {
            if (parseFloat(option.value) > 1) {
                option.disabled = isHeatingCircuit;
            }
        }
        
        if (isHeatingCircuit && parseFloat(fatorDemandaSelect.value) > 1) {
            fatorDemandaSelect.value = '1';
        }
    };
