...
function populateSoilResistivityDropdown(selectElement, soilData) {
    ...
    soilData.sort((a,b) => a.resistividade - b.resistividade).forEach(item => {
        ...
        option.textContent = `${item.resistividade} (${item.descricao})`;
        selectElement.appendChild(option);
    });
}
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <select id="metodoInstalacao-${id}">
                    <option value="A1">A1</option><option value="A2">A2</option><option value="B1" selected>B1</option><option value="B2">B2</option><option value="C">C</option><option value="D">D</option><option value="E">E</option><option value="F">F</option>
                </select>
            </div>
            <div class="form-group">
...
// O restante do arquivo (populateProjectList, populateFormWithProjectData, painéis de admin, cliente, PDF, etc.) permanece o mesmo da versão anterior.
    ...
    if(project.circuits_data) {
        project.circuits_data.forEach(savedCircuitData => {
            ...
            Object.keys(savedCircuitData).forEach(savedId => {
                if (savedId === 'id' || savedId.endsWith('_value')) return; // Ignora campos auxiliares
...
export function renderReport(calculationResults){ if(!calculationResults) return; const { feederResult, circuitResults } = calculationResults; const dataHora = (new Date).toLocaleString('pt-BR'); const formatLine = (label, value) => (label + ':').padEnd(30, ' ') + value; let reportText = `======================================================\n==           RELATORIO DE PROJETO ELETRICO           ==\n======================================================\n${formatLine('Gerado em', dataHora)}\n`; const reportData = feederResult.dados; reportText += `\n-- DADOS DA OBRA E CLIENTE --\n`; reportText += `${formatLine('Cliente', reportData.cliente || 'Nao informado')}\n`; reportText += `${formatLine(`Documento (${reportData.tipoDocumento})`, reportData.documento || 'Nao informado')}\n`; reportText += `${formatLine('Celular', reportData.celular || '-')}\n`; reportText += `${formatLine('Telefone', reportData.telefone || '-')}\n`; reportText += `${formatLine('E-mail', reportData.email || '-')}\n`; reportText += `${formatLine('Endereço do Cliente', reportData.enderecoCliente || '-')}\n`; reportText += `\n-- DADOS DA OBRA --\n`; reportText += `${formatLine('Código da Obra', reportData.projectCode || '-')}\n`; reportText += `${formatLine('Nome da Obra', reportData.obra || '-')}\n`; reportText += `${formatLine('Cidade da Obra', reportData.cidadeObra || '-')}\n`; reportText += `${formatLine('Endereço da Obra', reportData.enderecoObra || '-')}\n`; reportText += `${formatLine('Área da Obra (m²)', reportData.areaObra || '-')}\n`; reportText += `${formatLine('Unid. Residenciais', reportData.unidadesResidenciais || '-')}\n`; reportText += `${formatLine('Unid. Comerciais', reportData.unidadesComerciais || '-')}\n`; reportText += `${formatLine('Observações', reportData.observacoes || '-')}\n`; const respTecnico = document.getElementById('respTecnico').value; if (respTecnico) { reportText += `\n-- RESPONSAVEL TECNICO --\n`; reportText += `${formatLine('Nome', respTecnico)}\n`; reportText += `${formatLine('CREA', document.getElementById('crea').value)}\n`; } reportText += `\n-- QUADRO DE CARGAS RESUMIDO --\n`; reportText += `${formatLine(`Alimentador Geral`, `Carga Total: ${feederResult.calculos.potenciaDemandada.toFixed(2)} W`)}\n`; circuitResults.forEach(result => { reportText += `${formatLine(`Circuito ${result.dados.id}`, `${result.dados.nomeCircuito} - ${result.calculos.potenciaDemandada.toFixed(2)} W`)}\n`; }); const allCalculations = [feederResult, ...circuitResults]; allCalculations.forEach(result => { const { dados, calculos } = result; const title = dados.id === 'Geral' ? 'ALIMENTADOR GERAL' : `CIRCUITO ${dados.id}`; const correnteCorrigidaTexto = isFinite(calculos.correnteCorrigidaA) ? `${calculos.correnteCorrigidaA.toFixed(2)} A` : "Incalculável (Fator de Correção Zero)"; reportText += `\n\n======================================================\n==           MEMORIAL DE CALCULO - ${title.padEnd(16, ' ')} ==\n======================================================\n`; reportText += `\n-- PARÂMETROS DE ENTRADA --\n`; if (dados.id !== 'Geral') { reportText += `${formatLine('Nome do Circuito', dados.nomeCircuito)}\n`; reportText += `${formatLine('Tipo de Circuito', dados.tipoCircuito)}\n`; } reportText += `${formatLine('Potencia Instalada', `${calculos.potenciaInstalada.toFixed(2)} W`)}\n`; reportText += `${formatLine('Fator de Demanda Aplicado (%)', `${dados.fatorDemanda}%`)}\n`; reportText += `${formatLine('Fator de Potência', dados.fatorPotencia)}\n`; reportText += `${formatLine('Sistema de Fases', dados.fases)}\n`; reportText += `${formatLine('Tipo de Ligação', dados.tipoLigacao)}\n`; reportText += `${formatLine('Tensão (V)', `${dados.tensaoV} V`)}\n`; reportText += `${formatLine('Comprimento (m)', `${dados.comprimentoM} m`)}\n`; reportText += `${formatLine('Limite Queda de Tensão (%)', `${dados.limiteQuedaTensao} %`)}\n`; reportText += `\n-- ESPECIFICACOES DE INSTALAÇÃO E CORREÇÕES --\n`; reportText += `${formatLine('Material / Isolacao', `${dados.materialCabo} / ${dados.tipoIsolacao}`)}\n`; reportText += `${formatLine('Metodo de Instalacao', dados.metodoInstalacao)}\n`; reportText += `${formatLine('Temperatura Ambiente', `${dados.temperaturaAmbienteC}°C`)}\n`; if (dados.id !== 'Geral') { reportText += `${formatLine('Circuitos Agrupados', dados.numCircuitosAgrupados)}\n`; if (dados.resistividadeSolo && dados.resistividadeSolo > 0) { reportText += formatLine('Resist. do Solo (C.m/W)', dados.resistividadeSolo) + '\n'; } } if(calculos.fatorK1) { reportText += `${formatLine('Fatores de Correcao', `K1=${calculos.fatorK1.toFixed(2)}, K2=${calculos.fatorK2.toFixed(2)}, K3=${calculos.fatorK3.toFixed(2)}`)}\n`; } reportText += `\n-- RESULTADOS DE CÁLCULO E DIMENSIONAMENTO --\n`; reportText += `${formatLine('Corrente de Projeto (Ib)', `${calculos.correnteDemandada.toFixed(2)} A`)}\n`; reportText += `${formatLine('Corrente Corrigida (I\')', correnteCorrigidaTexto)}\n`; reportText += `${formatLine('Bitola Recomendada', `${calculos.bitolaRecomendadaMm2} mm²`)}\n`; reportText += `${formatLine('Queda de Tensao (DV)', `${calculos.quedaTensaoCalculada.toFixed(2)} % (Limite: ${dados.limiteQuedaTensao} %)`)}\n`; reportText += `${formatLine('Corrente Max. Cabo (Iz)', `${calculos.correnteMaximaCabo.toFixed(2)} A`)}\n`; reportText += `\n-- PROTECOES RECOMENDADAS --\n`; reportText += `${formatLine(`Disjuntor (${dados.tipoDisjuntor})`, `${calculos.disjuntorRecomendado.nome} (Icc: ${calculos.disjuntorRecomendado.icc} kA)`)}\n`; reportText += `${formatLine('Protecao DR 30mA', dados.requerDR ? `Sim (usar ${calculos.disjuntorRecomendado.nome.replace('A','')}A / 30mA)` : 'Nao')}\n`; reportText += `${formatLine('Protecao DPS', getDpsText(dados.dpsInfo))}\n`; reportText += `${formatLine('Eletroduto (aprox.)', `${calculos.dutoRecomendado} (${calculos.numCondutores} condutores)`)}\n`; }); document.getElementById('report').textContent = reportText.trim(); }
export function generatePdf(calculationResults, currentUserProfile) { if (!calculationResults) return; const { feederResult, circuitResults } = calculationResults; const { jsPDF } = window.jspdf; const doc = new jsPDF('p', 'mm', 'a4'); let yPos = 20; const leftMargin = 15; const valueMargin = 75; doc.setFont('helvetica', 'normal'); const addTitle = (title) => { doc.setFontSize(18); doc.setFont('helvetica', 'bold'); doc.text(title, 105, yPos, { align: 'center' }); yPos += 12; }; const addSection = (title) => { if (yPos > 260) { doc.addPage(); yPos = 20; } doc.setFontSize(12); doc.setFont('helvetica', 'bold'); doc.text(title, leftMargin, yPos); yPos += 8; }; const addLineItem = (label, value) => { if (yPos > 270) { doc.addPage(); yPos = 20; } doc.setFontSize(10); doc.setFont('helvetica', 'bold'); doc.text(label, leftMargin, yPos); doc.setFont('helvetica', 'normal'); doc.text(String(value || '-'), valueMargin, yPos); yPos += 6; }; addTitle("RELATÓRIO DE PROJETO ELÉTRICO"); const reportData = feederResult.dados; addSection("DADOS DO CLIENTE"); addLineItem("Cliente:", reportData.cliente); addLineItem(`Documento (${reportData.tipoDocumento}):`, reportData.documento); addLineItem("Celular:", reportData.celular); addLineItem("Telefone:", reportData.telefone); addLineItem("E-mail:", reportData.email); addLineItem("Endereço do Cliente:", reportData.enderecoCliente); yPos += 5; addSection("DADOS DA OBRA"); addLineItem("Código da Obra:", reportData.projectCode); addLineItem("Nome da Obra:", reportData.obra); addLineItem("Cidade da Obra:", reportData.cidadeObra); addLineItem("Endereço da Obra:", reportData.enderecoObra); addLineItem("Área da Obra (m²):", reportData.areaObra); addLineItem("Unid. Residenciais:", reportData.unidadesResidenciais); addLineItem("Unid. Comerciais:", reportData.unidadesComerciais); addLineItem("Observações:", reportData.observacoes); yPos += 5; addSection("INFORMAÇÕES DO RESPONSÁVEL TÉCNICO"); addLineItem("Nome:", document.getElementById('respTecnico').value); addLineItem("Título:", document.getElementById('titulo').value); addLineItem("CREA:", document.getElementById('crea').value); yPos += 5; addSection("INFORMAÇÕES DO RELATÓRIO"); const dataFormatada = new Date().toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' }); addLineItem("Gerado em:", dataFormatada); addLineItem("Gerado por:", currentUserProfile?.nome || 'N/A'); yPos += 5; addSection("RESUMO DA ALIMENTAÇÃO GERAL"); const feederBreakerType = feederResult.dados.tipoDisjuntor.includes('Caixa Moldada') ? 'MCCB' : 'DIN'; const feederBreakerText = `${feederBreakerType} ${feederResult.calculos.disjuntorRecomendado.nome}`; const feederHead = [['Carga Total', 'Tensão/Fases', 'Disjuntor Geral', 'DR', 'DPS', 'Cabo (Isolação)', 'Eletroduto']]; const feederBody = [[ `${feederResult.calculos.potenciaDemandada.toFixed(2)} W`, `${feederResult.dados.tensaoV}V - ${feederResult.dados.fases}`, feederBreakerText, feederResult.dados.requerDR ? 'Sim' : 'Nao', getDpsText(feederResult.dados.dpsInfo), `${feederResult.calculos.bitolaRecomendadaMm2} mm² (${feederResult.dados.tipoIsolacao})`, feederResult.calculos.dutoRecomendado ]]; doc.autoTable({ startY: yPos, head: feederHead, body: feederBody, theme: 'grid', headStyles: { fillColor: [44, 62, 80] }, styles: { fontSize: 8 } }); yPos = doc.lastAutoTable.finalY + 10; if (circuitResults.length > 0) { addSection("RESUMO DOS CIRCUITOS"); const head = [['Ckt', 'Nome', 'Pot. (W)', 'Disjuntor', 'DR', 'DPS', 'Cabo (Isolação)', 'Eletroduto']]; const body = circuitResults.map(r => { const circuitBreakerType = r.dados.tipoDisjuntor.includes('Caixa Moldada') ? 'MCCB' : 'DIN'; const circuitBreakerText = `${circuitBreakerType} ${r.calculos.disjuntorRecomendado.nome}`; return [ r.dados.id, r.dados.nomeCircuito, r.calculos.potenciaDemandada.toFixed(2), circuitBreakerText, r.dados.requerDR ? 'Sim' : 'Nao', getDpsText(r.dados.dpsInfo), `${r.calculos.bitolaRecomendadaMm2} mm² (${r.dados.tipoIsolacao})`, r.calculos.dutoRecomendado ]; }); doc.autoTable({ startY: yPos, head: head, body: body, theme: 'grid', headStyles: { fillColor: [44, 62, 80] }, styles: { fontSize: 8 } }); } const allCalculationsForMemorial = [feederResult, ...circuitResults]; allCalculationsForMemorial.forEach(result => { doc.addPage(); yPos = 20; const { dados, calculos } = result; const correnteCorrigidaTexto = isFinite(calculos.correnteCorrigidaA) ? `${calculos.correnteCorrigidaA.toFixed(2)} A` : "Incalculável"; const title = dados.id === 'Geral' ? `MEMORIAL DE CÁLCULO - ALIMENTADOR GERAL` : `MEMORIAL DE CÁLCULO - CIRCUITO ${dados.id}: ${dados.nomeCircuito}`; addTitle(title); addSection("-- PARÂMETROS DE ENTRADA --"); if (dados.id !== 'Geral') { addLineItem("Tipo de Circuito:", dados.tipoCircuito); } addLineItem("Potência Instalada:", `${calculos.potenciaInstalada.toFixed(2)} W`); addLineItem("Fator de Demanda:", `${dados.fatorDemanda}%`); addLineItem("Fator de Potência:", dados.fatorPotencia); addLineItem("Sistema de Fases:", dados.fases); addLineItem("Tipo de Ligação:", dados.tipoLigacao); addLineItem("Tensão (V):", `${dados.tensaoV} V`); addLineItem("Comprimento:", `${dados.comprimentoM} m`); addLineItem("Limite Queda de Tensão:", `${dados.limiteQuedaTensao}%`); yPos += 5; addSection("-- ESPECIFICAÇÕES DE INSTALAÇÃO E CORREÇÕES --"); addLineItem("Material / Isolação:", `${dados.materialCabo} / ${dados.tipoIsolacao}`); addLineItem("Método de Instalação:", dados.metodoInstalacao); addLineItem("Temperatura Ambiente:", `${dados.temperaturaAmbienteC}°C`); if (dados.id !== 'Geral') { addLineItem("Circuitos Agrupados:", dados.numCircuitosAgrupados); if (dados.resistividadeSolo && dados.resistividadeSolo > 0) { addLineItem("Resist. do Solo (C.m/W):", dados.resistividadeSolo); } } yPos += 5; addSection("-- RESULTADOS DE CÁLCULO E DIMENSIONAMENTO --"); addLineItem("Corrente de Projeto (Ib):", `${calculos.correnteDemandada.toFixed(2)} A`); addLineItem("Corrente Corrigida (I'):", correnteCorrigidaTexto); addLineItem("Bitola Recomendada:", `${calculos.bitolaRecomendadaMm2} mm²`); addLineItem("Queda de Tensão (DV):", `${calculos.quedaTensaoCalculada.toFixed(2)}%`); addLineItem("Corrente Máx. Cabo (Iz):", `${calculos.correnteMaximaCabo.toFixed(2)} A`); yPos += 5; addSection("-- PROTEÇÕES RECOMENDADAS --"); addLineItem("Disjuntor:", `${dados.tipoDisjuntor}: ${calculos.disjuntorRecomendado.nome} (Icc: ${calculos.disjuntorRecomendado.icc} kA)`); addLineItem("Proteção DR:", dados.requerDR ? `Sim (${calculos.disjuntorRecomendado.nome.replace('A','')}A / 30mA)` : 'Não'); addLineItem("Proteção DPS:", getDpsText(dados.dpsInfo)); }); doc.save(`Relatorio_${document.getElementById('obra').value || 'Projeto'}.pdf`); }