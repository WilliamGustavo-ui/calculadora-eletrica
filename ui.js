...
// CORREÇÃO 2 APLICADA (handleMainContainerInteraction)
...
export function handleMainContainerInteraction(event) {
    ...
    if (qdcBlock) {
        ...
        if (eventType === 'click') {
            ...
            // Verifica se o clique foi no header (qualquer parte dele)
            if (qdcHeader) {
                
                // VERIFICAÇÃO CORRIGIDA:
                // Ignora o clique APENAS se foi em um 'input', 'select' ou 'button'
                // (Os botões já são tratados acima, mas esta é uma garantia extra)
                if (target.closest('input, select, button')) {
                    return; // Foi um clique em um controle, não no header para expandir
                }

                // Se chegou aqui, foi um clique no fundo, label ou span (toggle-arrow)
