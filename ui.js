// Arquivo: ui.js (COMPLETO E CORRIGIDO - Remove 'as' TypeScript e mantém correção BTU/CV)
...
export function addCircuit(qdcId, savedCircuitData = null, circuitContainer = null) { 
    ...
    const circuitElement = newCircuitDiv.firstElementChild; // Remove o 'as HTMLElement'
    ...
    // Remove o 'as ... | null' - querySelector retorna Element | null por padrão
    ...
    if (savedCircuitData) { 
        Object.keys(savedCircuitData).forEach(key => { 
            ...
            if (el) { 
                // Remove o 'as ...'
                if (el.type === 'checkbox') { (el).checked = savedCircuitData[key]; } 
                else { (el).value = savedCircuitData[key]; } 
            } else { 
                ...
                if (elConstructed) { 
                    // Remove o 'as ...'
                    if (elConstructed.type === 'checkbox') { (elConstructed).checked = savedCircuitData[key]; } 
                    else { (elConstructed).value = savedCircuitData[key]; } 
                } 
            } 
        }); 
        // Remove o 'as ...'
...
// >>>>> CORREÇÃO: Função agora aceita elementos como parâmetros (sem 'as ...') <<<<<
        ...
        sortedQdcs.forEach(qdc => { const renderedQdcId = String(qdc.id); initializeQdcListeners(renderedQdcId); document.getElementById(`qdcFases-${renderedQdcId}`)?.dispatchEvent(new Event('change')); document.getElementById(`qdcTipoIsolacao-${renderedQdcId}`)?.dispatchEvent(new Event('change')); }); 
         ...
         setTimeout(() => { 
             sortedQdcs.forEach(qdc => { 
                ...
                if (qdc.circuits) {
                    qdc.circuits.forEach((circuit) => { // Removido 'any'
                        const circuitId = circuit.id;
                        const circuitElement = document.getElementById(`circuit-${circuitId}`); 
                        if (circuitElement) {
                            // Remove 'as ...'
                            const btuSelect = circuitElement.querySelector(`#potenciaBTU-${circuitId}`); 
                            const cvSelect = circuitElement.querySelector(`#potenciaCV-${circuitId}`); 
                            const btuGroup = circuitElement.querySelector(`#potenciaBTU_group-${circuitId}`); 
                            const cvGroup = circuitElement.querySelector(`#potenciaCV_group-${circuitId}`); 
