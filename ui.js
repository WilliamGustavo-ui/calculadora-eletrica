...
// ATUALIZADO: Agora filtra a lista única de fatores K1
export function setupDynamicTemperatures(techData) {
    if (techData?.fatores_k1) {
        tempOptions.pvc = techData.fatores_k1
            .filter(f => f.isolacao === 'PVC' && f.fator > 0)
            .map(f => f.temperatura_c)
            .sort((a, b) => a - b);
        
        tempOptions.epr = techData.fatores_k1
            .filter(f => f.isolacao === 'EPR' && f.fator > 0)
            .map(f => f.temperatura_c)
            .sort((a, b) => a - b);
    }
}
...
// ... (Resto do arquivo é idêntico e omitido para brevidade, mas deve ser mantido no seu arquivo)
// ... (populateProjectList, populateFormWithProjectData, painel admin, relatórios, etc.)
// ... (O código completo já foi fornecido em interações anteriores, esta seção apenas destaca a mudança principal)
