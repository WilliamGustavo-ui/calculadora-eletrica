...
function getFullFormData(forSave = false) {
    ...
    const feederData = {}; // Para salvar
    const feederDataForCalc = { id: 'feeder', nomeCircuito: "Alimentador Geral" }; // Para calcular
    document.querySelectorAll('#feeder-form input, #feeder-form select').forEach(el => {
        const value = el.type === 'checkbox' ? el.checked : el.value;
        feederData[el.id] = value; // Salva valor original
        const key = el.id.replace('feeder', '').charAt(0).toLowerCase() + el.id.replace('feeder', '').slice(1);
        // Conversão robusta para cálculo
        let calcValue = value;
        if (el.type === 'number' || ['fatorDemanda', 'fatorPotencia', 'comprimentoM', 'limiteQuedaTensao'].includes(key)) {
            ...
            calcValue = parseFloat(value) || 0;
             ...
             calcValue = el.checked;
        }
        feederDataForCalc[key] = calcValue;
    });

    const qdcsData = []; // Para salvar
    const allCircuitsForCalc = []; // Para calcular
    ...
    document.querySelectorAll('#qdc-container .qdc-block').forEach(qdcBlock => {
        ...
        qdcBlock.querySelectorAll('.qdc-config-grid input, .qdc-config-grid select').forEach(el => {
            qdcConfigData[el.id] = el.type === 'checkbox' ? el.checked : el.value;
        });
        ...
        qdcBlock.querySelectorAll('.circuit-block').forEach(circuitBlock => {
            ...
            const circuitData = { id: circuitId }; // Para salvar
            const circuitDataForCalc = { qdcId: qdcId, id: circuitId }; // Para calcular

            circuitBlock.querySelectorAll('input, select').forEach(el => {
                const value = el.type === 'checkbox' ? el.checked : el.value;
                circuitData[el.id] = value; // Salva valor original
                const key = el.id.replace(`-${circuitId}`, '');
                // Conversão robusta para cálculo
                let calcValue = value;
                 if (el.type === 'number' || ['potenciaW', 'fatorDemanda', 'fatorPotencia', 'comprimentoM', 'limiteQuedaTensao'].includes(key)) {
                    calcValue = parseFloat(value) || 0;
    ...
    // Objeto para Edge Function
    return { mainData, feederData: feederDataForCalc, circuitsData: allCircuitsForCalc, clientProfile };
}
...
async function handleAdminUserActions(event) {
    ...
    try {
        ...
        if (target.classList.contains('remove-user-btn')) { if (confirm('ATENÇÃO: Ação irreversível! Excluir este usuário permanentemente?')) { const { data, error } = await api.deleteUserFromAdmin(userId); if (error) { throw error; } alert(data?.message || 'Usuário excluído com sucesso.'); await showAdminPanel(); } }
         ...
         await showAdminPanel(); // Recarrega
    }
}
    ...
    try {
        ...
        if (functionError) {
            ...
            try {
                 ...
                 if (errorDetails?.error) { errMsg = errorDetails.error; }
                 console.error("Detalhes do erro da Edge Function:", errorDetails || functionError.context?.statusText);
            } catch(e) { console.error("Erro ao parsear detalhes do erro da Edge Function:", e); console.error("Contexto original do erro:", functionError.context); }
        ...
        console.log("Resultados recebidos da Edge Function:", results);
...
function setupEventListeners() {
    ...
    if(appContainer) {
        // Listener geral para interações DENTRO dos QDCs/Circuitos
    ...
    document.getElementById('changeClientBtn').addEventListener('click', async () => {
        try { allClients = await api.fetchClients(); ui.populateSelectClientModal(allClients, true);} catch(e){ alert("Erro ao carregar clientes.")}
    });
