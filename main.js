...
async function handleClientListClick(event) {
    ...
    if (target.classList.contains('edit-client-btn')) { const clientToEdit = allClients.find(c => c.id == clientId); if (clientToEdit) ui.openEditClientForm(clientToEdit); }
    if (target.classList.contains('delete-client-btn')) { if (confirm('Tem certeza que deseja excluir este cliente?')) { const { error } = await api.deleteClient(clientId); if (error) { alert('Erro ao excluir cliente: ' + error.message); } else { await handleOpenClientManagement(); } } }
}

async function handleNewProject(showModal = true) {
    if (showModal) { allClients = await api.fetchClients(); ui.populateSelectClientModal(allClients); } else { ui.resetForm(true, null); }
}
...
async function handleSaveProject() {
    ...
    const mainData = {};
    document.querySelectorAll('#main-form input, #main-form textarea, #main-form select').forEach(el => { if (el.id && !['currentProjectId', 'currentClientId'].includes(el.id)) { mainData[el.id] = el.value; } });
    const techData = {};
...
async function handleProjectPanelClick(event) {
    ...
    if (target.classList.contains('transfer-owner-btn')) {
        ...
        if (newOwnerId && confirm('Tem certeza que deseja transferir a propriedade desta obra? Você perderá o acesso a ela se transferir para outro usuário.')) {
...
function main() { setupEventListeners(); }
main();
...
supabase.auth.onAuthStateChange(async (event, session) => {
    if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN') {
        ...
        currentUserProfile = null; technicalData = null; ui.showLoginView();
